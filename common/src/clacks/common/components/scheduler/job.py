...
from clacks.common.components.scheduler.util import to_unicode, ref_to_obj, get_callable_name,\
    obj_to_ref
...
class Job(object):
    ...
    def get_run_times(self, now):
        ...
        # There's no next run time if we're paused
    ...
    def __getstate__(self):
        ...
        state['callback_ref'] = obj_to_ref(self.callback) if self.callback else None
        return state
